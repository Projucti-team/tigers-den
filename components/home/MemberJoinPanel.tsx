...
export function MemberJoinPanel({ authConfigured }: MemberJoinPanelProps) {
  ...
  if (!authConfigured) {
    return (
      <p className="mx-auto max-w-md text-center text-sm text-charcoal/75">
        Social sign-in is being set up. Add Google and Facebook OAuth keys to your environment
        (see <code className="text-xs">.env.example</code>) to enable joining.
      </p>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-md space-y-4">
        <p className="text-center text-sm text-charcoal/75">
          Sign in with Google or Facebook — we&apos;ll save your email and name automatically.
        </p>
        <button
          type="button"
          onClick={() => signIn("google", { callbackUrl: JOIN_CALLBACK })}
          className="flex w-full items-center justify-center gap-3 rounded border-2 border-charcoal/15 bg-white px-4 py-3 text-sm font-bold text-charcoal shadow-sm transition hover:border-emerald hover:shadow-md"
        >
          <GoogleIcon />
        ...
        <button
          type="button"
          onClick={() => signIn("facebook", { callbackUrl: JOIN_CALLBACK })}
          className="flex w-full items-center justify-center gap-3 rounded border-2 border-[#1877F2] bg-[#1877F2] px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#166fe5]"
        >
          <FacebookIcon />
